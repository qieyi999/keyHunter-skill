...
/**
 * 换源章节定位算法 (app 端 `BookHelp.getDurChapter` 及其私有辅助方法的 commonMain 下沉版)。
 *
 * 原滞留 jvmAndAndroidMain 仅因两个章节号正则用 `java.util.regex.Pattern`, 此处等价改写为
 * kotlin [Regex] (正则串一字未改), 全平台 (Android/JVM/iOS/鸿蒙) 共用同一份实现。
 */
object BookHelpChapterLocator {
    ...
    private fun getChapterNum(chapterName: String?): Int {
        ...
        return StringUtils.stringToInt(
            (
                chapterNamePattern1.find(chapterName1)
                    ?: chapterNamePattern2.find(chapterName1)
                )?.groups?.get(1)?.value
                ?: "-1"
        )
    }
