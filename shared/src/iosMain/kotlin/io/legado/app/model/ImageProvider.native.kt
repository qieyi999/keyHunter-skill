...
/**
 * [ImageProvider] 的 native (iOS) stub。
 *
 * iOS 端图片管线由 [io.legado.app.help.image.ReaderImageResolver] / [ReaderImageCache]
...
actual object ImageProvider {
    ...
    actual val errorBitmap: ImageProviderBitmap
        get() = error("ImageProvider not implemented on native; use ReaderImageResolver")
    ...
    ): File = error("ImageProvider not implemented on native; use ReaderImageResolver")

    actual suspend fun getImageSize(
        book: Book,
        src: String,
        bookSource: BookSource?
    ): ImageProviderSize = error("ImageProvider not implemented on native; use ReaderImageResolver")
    ...
    actual fun getImage(
        ...
        error("ImageProvider not implemented on native; use ReaderImageResolver")
